    ...
    if (api && courseId) {
      try {
        ...
        if (response.data.error) {
          ...
          console.log('New chat session started:', response.data);
          setMessages([]);
          setMessageCount(0);
  ...
  const loadChat = async (chatId) => {
    if (api && chatId) {
      try {
        ...
        console.log('load chat res', response);
      } catch (error) {
      ...
      <div className={`flex-grow transition-all duration-300 p-4 ${isSidebarOpen ? 'ml-3' : 'ml-7'}`}>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div>
            ...
            <button onClick={startChat} className="bg-red-500 text-white p-2 rounded hover:bg-red-600">
              Clear Conversation
            </button>
            <button onClick={saveCurrentChat} className="bg-green-500 text-white p-2 rounded hover:bg-green-600 mr-2">
            Save Chat
          </button>
          </div>
        </div>
